  ...
  app.innerHTML = `
    ...
    ${section('📋 Before a match', `
      ...
      <ul class="stack" style="margin:0; padding-left:18px;">
        ...
        <li><strong>Squad tab</strong> — the fast path on match day: tap players to mark them present, then build the lineup by tapping a pitch position and a player. "Use RSVP List" marks everyone who said they're In as present in one tap. A <strong>Formation</strong> picker sits above the pitch — a few common shapes per squad size come as default suggestions, and "Create your own in Settings" builds a custom one by simply choosing how many defenders/midfielders/forwards you want. Switching formations remaps your current lineup rather than wiping it (same-shaped slots keep their player). Positions keep tracking through the live match too — a substitute takes over the exact spot the player they replaced held, shown as a role label (DEF/MID/FWD) on their card.</li>
        ...
        <li><strong>🎲 Balance Teams</strong> (from RSVP/Squad, or Roster) randomly splits players into 2–4 teams with an even mix of skill streams. Not happy with a particular split? Tap a player on one team, then tap "Move here →" on another team to move them across by hand, without reshuffling everyone else.</li>
      </ul>
    `)}

    ${section('⚽ Running a live match', `
      <ul class="stack" style="margin:0; padding-left:18px;">
        <li>Tap <strong>▶ Start Game</strong> from the game page, then <strong>▶ Start Clock</strong> when kickoff happens. The clock keeps running even if you switch screens, lock your phone, or put it in your pocket — it's anchored to the actual time, not to how often the screen happens to be checking in, so it catches straight up the moment you look at it again instead of quietly falling behind while the screen was off. The big number always shows time elapsed in the <strong>current half/period only</strong>, starting fresh at 0:00 when a new period begins — so it can't look like it's carried on from wherever you left off in the last one. A smaller "Total" reading next to it still tracks the whole match if you need that too, and it's what "Full time" on a completed game shows. Starting a new period always <strong>pauses the clock</strong>, even if it was still running when the previous period ended — so the 2nd half never starts ticking on its own, and you always tap ▶ Start Clock again once play actually kicks back off.</li>
        <li>Log goals and GK saves with the buttons on the live screen. Every on-field player (and the goalkeeper) has a small "⋯" icon tucked in the corner of their card (deliberately understated, not a big red button, so it's not easy to catch by accident while tapping the card itself to make a sub) that opens a short "What happened?" menu — "Card / Remove" if Cards are enabled in Settings, or "Remove from Match" if not — instead of a single ambiguous "Send Off": 🟨 Yellow (stays on), 🟥 Red, 🚑 Injury, or Other reason (Cards off just offers Injury/Other, since there's nothing to log). A <strong>🟨 Card</strong> (or 🚑 Remove, Cards off) button also sits next to 🧤 GK Save for a quicker path — pick the player from a dropdown first, same as the Goalkeeper and Log Goal dialogs, rather than hunting for their own "⋯" icon; the menu that follows is the same one and defaults to Yellow. Anything other than a first yellow takes them off the pitch and out of the bench pool immediately — they can never be picked for a sub again this match. A <strong>second yellow card for the same player is applied automatically</strong> as a send-off the moment you log it, with a clear heads-up message, rather than leaving you to notice and remove them yourself.</li>
        ...
        <li>A live <strong>Formation</strong> pitch sits at the top of the screen, matching your lineup as it actually stands. Tap any on-field player on it to open a dropdown with two kinds of options: bring someone on from the bench (a normal substitute, straight on with no need to scroll down to the bench section first), or <strong>swap positions</strong> with another on-field teammate — both stay on, nothing about playing time or the bench changes, just which spot each one occupies (handy for a tactical reshuffle mid-match). An <strong>empty spot</strong> on the pitch (shown as a dashed circle with a "+") works the bring-on way too — tap it to bring someone straight into that exact position, no swap needed. The <strong>goalkeeper spot</strong> — filled or empty — always opens the Change/Assign Goalkeeper dialog instead of a plain substitute, so stint tracking stays correct. A <strong>Formation</strong> button above the pitch works mid-match too: it opens a picker (a proper pop-up, not an inline dropdown, so a busy match clock ticking every second in the background can't snap it shut on you) — switching shapes moves everyone's dot to the new layout without touching who's actually on the field. Right next to it, <strong>⚡ Auto-Fill Field</strong> fills any spare pitch spot (goalkeeper included) with present players in one tap — for when you started the match straight off marking the squad present, skipping the Lineup tab, and just want to get playing.</li>
        ...
        <li>The <strong>📋 Substitution Plan</strong> — your own manual rotation schedule, either set up before kickoff or built as you go — shows every planned swap with a "due in…" countdown (or "⏰ Due now" once its minute arrives) and a one-tap <strong>✅ Sub Now</strong> button that makes the actual substitution for you. It never subs anyone on its own — it's purely a reminder of your own plan, so you can add, edit, or delete entries any time, live or not. Each bench player also shows their own "🕐 due" line under their card when they're next up in the plan. If a planned entry has gone stale by the time you tap "Sub Now" — the incoming player has since become goalkeeper, been sent off, or is already on — nothing happens except a message explaining why; the app never lets the same player show up on the field twice at once, so the entry stays in the plan for you to fix or delete.</li>
        <li><strong>End Game</strong> finishes the match (works fine mid-period too, for an early end). <strong>End & Next</strong> appears once a same-day second match exists, and carries fair-play minutes over so the whole day stays balanced, not just one match. Both ask you to confirm first — a clearly-labeled "Yes, End Game"/"Yes, End & Next" button, not just a generic OK — and if you confirm by mistake, a completed match's screen shows an <strong>↩️ Undo</strong> button that makes it live again with everything you'd already logged (score, subs, cards, the works) still intact.</li>
        <li>To keep the live screen from becoming a wall of text as the match goes on, <strong>Playing Time</strong> and <strong>Match Events</strong> sit collapsed by default — tap either heading any time to expand it.</li>
      </ul>
    `)}
    ...
    ${section('🏆 After the match', `
      <ul class="stack" style="margin:0; padding-left:18px;">
        <li>Every completed match gets a <strong>Match Summary</strong> — tap "View Summary" from the game's page. It distills the whole match into ⚽ scorers (with assists), 🧤 saves, and 🟨 cards, so you don't have to read back through the full chronological log to see who did what; that raw log is still there below, just collapsed by default.</li>
    ...
    ${section('🏃 Training sessions', `
      ...
      <ul class="stack" style="margin:0; padding-left:18px;">
        ...
        <li><strong>Rotating stations</strong> — tick "Rotate groups through each activity" on a grouped block to run it as a circuit: groups work through stations in turn rather than staying on just one. Stations are their own list — "+ Add Station"/🗑 to add or remove — separate from how many groups you have, and a <strong>Number of rotations</strong> field lets you set how many legs actually run (defaults to the number of stations, so everyone visits each one once). Fewer stations than groups is exactly how two (or more) groups end up sharing a station at the same time — e.g. 4 groups over 3 stations means one station always runs with two groups together — with no separate "pairing" step needed. The minutes field is "per rotation," and the block's total time (and the session's running total) scales to minutes × number of rotations.</li>
    ...
    ${section('📚 Drill Library', `
      ...
      <ul class="stack" style="margin:0; padding-left:18px;">
        ...
        <li>When building a session's Plan, each activity field can pull a drill's name straight in via its "📚 Fill from Drill Library…" dropdown — this also keeps a link back to that drill, so a "📚 View Drill" button shows up next to the activity (on the Plan tab, and on the live session timer once it's running) to reopen its full description, image, PDF, and tags without leaving Training. Typing over the activity text by hand drops that link, since the text no longer matches — but if what you typed (or an older block from before this existed) happens to match a saved drill's name exactly, "View Drill" still shows up for it automatically.</li>
